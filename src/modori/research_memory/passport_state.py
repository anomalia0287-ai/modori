...
class PassportHistory:
    ...
    def inspect(
        ...
        for expected_sequence, event in enumerate(events, start=1):
            ...
            try:
                event.verify()
                resulting_snapshot_id = event.require_typed_artifact_subjects(lookup)
            except (LedgerContractError, ValueError, TypeError) as exc:
    ...
    def _fold_passport_commit(
        ...
        passport_artifact_id = event.payload["passport_artifact_id"]
    ...
    def _fold_clarification_answer(
        ...
        answer = _decode_artifact(
            event.payload["answer_artifact_id"],
            LedgerArtifactKind.CLARIFICATION_ANSWER,
            lookup,
        )
        evidence = _decode_artifact(
            event.payload["evidence_ref_artifact_id"],
    ...
    def _fold_retraction(
        ...
        retracted_event_id = event.payload["retracted_event_id"]
        matches = tuple(
            index
            for index, record in enumerate(records)
