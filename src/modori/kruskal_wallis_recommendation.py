...
_DEPENDENT_MEASURES = {"scale", "ordinal"}
_GROUP_MEASURES = {"nominal", "ordinal"}
_MAX_GROUPS = 12


@dataclass(frozen=True)
class KruskalWallisEligibilityProvider:
    module_key: str = "kruskal_wallis"
    ...
    def candidates(
        ...
        *,
        ...
        dependents = self._numeric_keys(dataset, frame, _DEPENDENT_MEASURES)
        groups = self._group_keys(dataset, frame)
        for group_key in groups:
            for dependent_key in dependents:
                if dependent_key == group_key:
                    continue
                if not self._has_kruskal_shape(
                    frame,
    ...
    def _is_usable_numeric(series: pd.Series, column: str) -> bool:
        if KruskalWallisEligibilityProvider._is_identifier_like(column):
            return False
    ...
    @staticmethod
    def _has_kruskal_shape(
        frame: pd.DataFrame,
        *,
        dependent_key: str,
        group_key: str,
    ) -> bool:
        values = frame.loc[:, [dependent_key, group_key]].copy()
        values[dependent_key] = pd.to_numeric(values[dependent_key], errors="coerce")
        values = values.dropna(axis=0, how="any")
        if values.empty:
            return False
        counts = values.groupby(group_key, dropna=True)[dependent_key].count()
        if len(counts) < 3:
            return False
        return bool((counts >= 2).all())


def eligibility_provider() -> KruskalWallisEligibilityProvider:
    return KruskalWallisEligibilityProvider()
