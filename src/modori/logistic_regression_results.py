...
from modori.value_tokens import display_label_key, normalized_value_key
...
LOGISTIC_WARNING_TEXTS: dict[str, dict[str, str]] = {
    ...
    "predictor_odds_ratio_unrepresentable": {
        "ko": "일부 예측변수의 1단위 변화 승산비가 부동소수점 표현 범위를 벗어났습니다. 계수와 추론은 유지했으며 예측변수를 해석 가능한 단위로 재조정하십시오.",
        "en": "One or more one-unit predictor odds ratios exceed the floating-point representation range. Coefficients and inference were retained; rescale the predictors to interpretable units.",
    },
    ...
    def __post_init__(self) -> None:
        ...
        if self.odds_ratio is None or self.odds_ratio_ci is None:
            if self.odds_ratio is not None or self.odds_ratio_ci is not None:
                raise ValueError(
                    "Logistic odds ratio and CI must both be defined or undefined"
                )
            odds_ratio = None
            odds_ratio_ci = None
        else:
        ...
        if se <= 0 or chi_square < 0 or (odds_ratio is not None and odds_ratio <= 0):
            raise ValueError(
                "Logistic coefficient SE and defined odds ratio must be positive"
            )
        if not math.isclose(
            chi_square, z_value * z_value, rel_tol=1e-10, abs_tol=1e-12
        ):
        ...
        if odds_ratio is not None and odds_ratio_ci is not None:
            ...
            if not all(
                ...
                for actual, expected in zip(
                    odds_ratio_ci, expected_odds_ci, strict=True
                )
            ):
                raise ValueError(
                    "Logistic odds-ratio CI must equal exp(coefficient CI)"
                )
...
class BinaryClassificationTable:
    ...
    def __post_init__(self) -> None:
        ...
        if sum(counts) == 0:
            raise ValueError(
                "Classification confusion counts must include observations"
            )
        for field_name in (
            "sensitivity",
            "specificity",
        ...
        expected_metrics = {
            ...
            else counts[3] / (counts[3] + counts[2]),
            "specificity": None
            if counts[0] + counts[1] == 0
            else counts[0] / (counts[0] + counts[1]),
            "positive_predictive_value": (
                None
                if counts[3] + counts[1] == 0
                else counts[3] / (counts[3] + counts[1])
            ),
            "negative_predictive_value": (
                None
                if counts[0] + counts[2] == 0
                else counts[0] / (counts[0] + counts[2])
            ),
        ...
        for field_name, expected in expected_metrics.items():
            ...
            if actual is None or expected is None:
                if actual is not expected:
                    raise ValueError(
                        f"Classification {field_name} is inconsistent with counts"
                    )
...
class CalibrationBin:
    ...
    def __post_init__(self) -> None:
        ...
        if not math.isclose(
            self.observed_rate, events / count, rel_tol=1e-12, abs_tol=1e-12
        ):
            raise ValueError(
                "Calibration observed rate is inconsistent with event count"
            )
...
class LogisticRegressionResult:
    ...
    def __post_init__(self) -> None:
        ...
        if normalized_value_key(self.event_value) == normalized_value_key(
            self.non_event_value
        ):
            raise ValueError("Logistic outcome values must be distinct")
        event_label_key = display_label_key(self.event_label)
        non_event_label_key = display_label_key(self.non_event_label)
        if (
            not event_label_key
            or not non_event_label_key
            or event_label_key == non_event_label_key
        ):
            raise ValueError(
                "Logistic outcome display labels must be non-blank and distinguishable"
            )
        counts = tuple(
            _count(value, label)
            for value, label in (
        ...
        if self.classification.tn + self.classification.fp != self.non_event_count:
            raise ValueError(
                "Logistic classification non-event counts are inconsistent"
            )
        ...
        undefined_predictor_odds = any(
            row.term_type != "intercept" and row.odds_ratio is None
            for row in self.coefficients
        )
        predictor_warning = "predictor_odds_ratio_unrepresentable" in self.warning_codes
        if undefined_predictor_odds != predictor_warning:
            raise ValueError(
                "Undefined predictor odds ratios must align with the disclosure warning"
            )
        for field_name in (
            "log_likelihood",
            "null_log_likelihood",
