...
def _require_schema(
    ...
    *,
    ...
    if (
        ...
        raise IntegrationVerificationError(
            f"invalid schema: expected {expected} v{version}"
        )
...
def _validate_ledger(
    ...
    _require_schema(ledger, LEDGER_SCHEMA, version=2)
    ...
    branch_only: dict[str, tuple[str, object]] = {}
    for side, key, blob_key in (
        ...
        for record in _records(manifest, key):
            branch_only[str(record["path"])] = (side, record[blob_key])

    adjustments = _records(ledger, "branch_only_adjustments")
    adjustment_paths = [str(item.get("path", "")) for item in adjustments]
    if len(adjustment_paths) != len(set(adjustment_paths)):
        raise IntegrationVerificationError(
            "duplicate branch-only adjustment path"
        )
    for adjustment in adjustments:
        path = str(adjustment.get("path", ""))
        expected_source = branch_only.get(path)
        if expected_source is None:
            raise IntegrationVerificationError(
                f"invalid branch-only adjustment path: {path}"
            )
        expected_side, expected_blob = expected_source
        if adjustment.get("source_side") != expected_side:
            raise IntegrationVerificationError(
                f"branch-only adjustment source mismatch: {path}"
            )
        if adjustment.get("source_blob_oid") != expected_blob:
            raise IntegrationVerificationError(
                f"branch-only adjustment source blob mismatch: {path}"
            )
        integrated_blob = adjustment.get("integrated_blob_oid")
        if not isinstance(integrated_blob, str) or not SHA_PATTERN.fullmatch(
            integrated_blob
        ):
            raise IntegrationVerificationError(
                f"invalid branch-only adjustment blob: {path}"
            )
        for field in ("required_presences", "focused_tests"):
            value = adjustment.get(field)
            if not isinstance(value, list) or not value:
                raise IntegrationVerificationError(
                    f"incomplete branch-only adjustment: {path}"
                )
        if not isinstance(adjustment.get("required_absences"), list):
            raise IntegrationVerificationError(
                f"incomplete branch-only adjustment: {path}"
            )
        if (
            not str(adjustment.get("rationale", "")).strip()
            or adjustment.get("status") != "verified"
        ):
            raise IntegrationVerificationError(
                f"incomplete branch-only adjustment: {path}"
            )
...
def verify_integration(
    ...
    repo = repo.resolve()
    _require_schema(manifest, MANIFEST_SCHEMA)
    entries, adjustments = _validate_ledger(manifest, ledger)
    ...
    adjustments_by_path = {
        str(item["path"]): item for item in adjustments
    }
    ...
    for side, key, blob_key in (
        ...
        for record in _records(manifest, key):
            ...
            actual_blob = _blob(repo, merge, path)
            expected_blob = record[blob_key]
            adjustment = adjustments_by_path.get(path)
            if actual_blob == expected_blob:
                if adjustment is not None:
                    raise IntegrationVerificationError(
                        f"unnecessary branch-only adjustment: {path}"
                    )
                continue
            if adjustment is None:
                blob_mismatches.append(f"{side}:{path}")
                continue
            if adjustment["integrated_blob_oid"] != actual_blob:
                raise IntegrationVerificationError(
                    f"branch-only adjustment blob mismatch: {path}"
                )
    ...
    return {
        ...
        "branch_only_adjustment_count": len(adjusted_paths),
        "adjusted_branch_only_paths": sorted(adjusted_paths),
