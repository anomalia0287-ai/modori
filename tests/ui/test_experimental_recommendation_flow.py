...
def test_select_and_prepare_do_not_touch_pipeline_or_worker() -> None:
    ...
    assert controller.lastMessage == "검토할 분석 후보를 선택했습니다."
...
def test_guide_uses_explicit_experimental_confirmation_flow() -> None:
    ...
    assert "uiController.selectedRecommendationFieldText(key)" in source
    assert "uiController.preparedRecommendationField(" not in source
