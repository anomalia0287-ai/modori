...
def _fixture_repo(
    ...
    *,
    ...
    replace_research_only_path: bool = False,
    ...
    if replace_research_only_path:
        _write(repo, "research-only.txt", "integrated adjustment\n")
...
def _research_only_adjustment(
    ...
    source = next(
        item
        for item in manifest["research_only"]
        if item["path"] == "research-only.txt"
    )
    return {
        "path": "research-only.txt",
        "source_side": "research",
        "source_blob_oid": source["research_blob_oid"],
        "integrated_blob_oid": _git(
            repo,
            "rev-parse",
            f"{merge}:research-only.txt",
        ),
        "rationale": "integration requires a bounded contract adaptation",
        "required_presences": ["integrated behavior"],
        "required_absences": ["unreviewed source loss"],
        "focused_tests": ["tests/test_shared.py"],
        "status": "verified",
    }
...
def test_initial_ledger_v2_has_closed_branch_only_adjustment_list(
    git_tmp_path: Path,
) -> None:
    repo, release, research, _ = _fixture_repo(git_tmp_path)
    manifest = build_source_manifest(repo, release, research)
    ...
    assert ledger["branch_only_adjustments"] == []


def test_verifier_accepts_exactly_ledgered_branch_only_adjustment(
    git_tmp_path: Path,
) -> None:
    repo, release, research, merge = _fixture_repo(
        git_tmp_path,
        replace_research_only_path=True,
    )
    manifest = build_source_manifest(repo, release, research)
    ledger = _completed_ledger(manifest)
    ledger["branch_only_adjustments"] = [
        _research_only_adjustment(repo, manifest, merge)
    ]

    report = verify_integration(repo, manifest, ledger, merge)

    assert report["branch_only_adjustment_count"] == 1
    assert report["adjusted_branch_only_paths"] == ["research-only.txt"]
    assert report["branch_only_blob_mismatches"] == []


def test_verifier_rejects_forged_branch_only_adjustment_blob(
    git_tmp_path: Path,
) -> None:
    repo, release, research, merge = _fixture_repo(
        git_tmp_path,
    ...
    manifest = build_source_manifest(repo, release, research)
    ledger = _completed_ledger(manifest)
    adjustment = _research_only_adjustment(repo, manifest, merge)
    adjustment["integrated_blob_oid"] = "0" * 40
    ledger["branch_only_adjustments"] = [adjustment]

    with pytest.raises(
        IntegrationVerificationError,
        match="branch-only adjustment blob mismatch",
    ):
        verify_integration(repo, manifest, ledger, merge)


def test_verifier_rejects_unnecessary_branch_only_adjustment(
    git_tmp_path: Path,
) -> None:
    repo, release, research, merge = _fixture_repo(git_tmp_path)
    manifest = build_source_manifest(repo, release, research)
    ...
    with pytest.raises(
        IntegrationVerificationError,
        match="unnecessary branch-only adjustment",
    ):
        verify_integration(repo, manifest, ledger, merge)
