...
| Reliability | McDonald's omega has a committed R `psych::omega` reproduction path with captured stdout and no hard-coded fabricated R number. | `tests/r/omega_reference.R`; `tests/r/omega_reference.stdout.txt`; `tests/test_reliability_step.py::test_mcdonald_omega_matches_r_psych_when_r_is_available` using `MODORI_RSCRIPT=.tools\r-env\Scripts\Rscript.exe` | pass |
...
| Dependency security | `pip check` reports no broken requirements; `pip-audit --local` reports no known vulnerabilities except local package skip. | `.venv\Scripts\python.exe -m pip check`; `.venv\Scripts\python.exe -m pip_audit --local --cache-dir .pip-audit-cache --progress-spinner off` | pass as of 2026-06-29 latest run |
