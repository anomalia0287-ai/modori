...
비슷한 이름의 이전 키트는 사용하지 않는다. `.zip.sha256` 내용과 채팅에 고정된
...
4. 고정된 release 프로토콜만 실행한다.
5. 모든 raw observation을 봉인한 뒤에만 JSON·sidecar·한국어 요약을 게시한다.
...
사전검증 또는 실행 오류가 났다면 다음도 가져온다.

- `bootstrap-error-<UTC>.txt`
- 오류 코드가 보이는 화면 사진(선택)

`work` 폴더, 프로젝트 원장, 노트북의 다른 파일은 가져오지 않는다.

## 8. 오류와 판정의 구분

- `invalid_run`: 해시·파일 집합·프로토콜·AC·경로·runtime identity·raw observation
  완전성 중 하나가 유효하지 않다. 제품 성능 실패로 세지 않으며, 합리적인 설정
  오류를 고친 뒤 새 폴더에서 다시 실행할 수 있다.
