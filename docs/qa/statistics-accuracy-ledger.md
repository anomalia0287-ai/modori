...
| `rank_based_nonparametric_tests` | Current coverage spans Mann-Whitney, Wilcoxon, Kruskal-Wallis, Friedman, and Spearman through module tests. | Mann-Whitney records tie policy and exact-vs-asymptotic selection; Wilcoxon records zero-difference, tie, correction, and method policy; rank methods also appear in Kruskal-Wallis, Friedman, and correlation modules. | Cross-engine disagreements against R/SPSS/JASP are not ledger-complete. Kruskal-Wallis, Friedman, and Spearman still need dedicated Likert-heavy tie-policy fixtures. | Add R/SPSS/JASP anchors, Kruskal-Wallis/Friedman/Spearman tied-rank fixtures, and small-sample warning policy. |
...
- Current numerical-policy focus command:
  `pytest -p no:cacheprovider tests/test_regression_step.py tests/test_regression_categorical_interaction.py tests/test_mediation_step.py tests/test_moderated_mediation_step.py tests/test_compare_groups_step.py tests/test_paired_comparison_step.py tests/test_nist_strd_fixtures.py tests/test_tail_probability_policy.py -q`
