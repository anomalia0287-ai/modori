...
| `regression_ols` | Module tests cover OLS coefficients, categorical interaction behavior, report output, validation behavior, and NIST StRD Longley certified values. | Categorical interaction fixture, report fixture, singular predictor rejection paths, condition-number diagnostics, ill-conditioned design rejection. | Broader diagnostics parity is not yet ledger-complete. NIST StRD coverage currently starts with Longley only. | Add NIST Filip/Wampler fixtures, heteroskedasticity fixtures, and missing-row golden fixtures. |
