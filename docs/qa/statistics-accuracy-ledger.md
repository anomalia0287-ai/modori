...
| `mediation` | Independent OLS coefficient calculation, deterministic bootstrap reproduction for the current percentile method, and slow known-effect coverage smoke. | Schema migration, duplicate role rejection, non-scale rejection, unsupported standardization rejection, singular model rejection, OLS condition-number rejection, SVD-backed OLS covariance without normal-equation inverse, user-facing bootstrap default `5000`, warning below `1000`, slow `1000`-resample known-effect coverage smoke behind `scripts/slow_stats_gate.py`. | Supports observed-variable simple mediation only; serial/latent mediation is outside current scope. The slow coverage smoke is adequacy evidence for gross regression detection, not exhaustive interval proof or cross-engine parity. | Add missing-row parity, no-covariate parity, controlled-index cross-engine mediation references, and broader coverage designs if the product scope expands. |
| `moderated_mediation` | Independent OLS point-effect references for PROCESS-style Model 7 and Model 14; deterministic bootstrap reproduction for Model 7; slow known-effect coverage smoke for the Model 7 index. | Schema migration, unsupported model rejection, duplicate role rejection, non-scale rejection, unsupported centering rejection, OLS interaction condition-number rejection, shared SVD-backed OLS covariance helper, user-facing bootstrap default `5000`, warning below `1000`, slow Model 7 index coverage smoke behind `scripts/slow_stats_gate.py`. | Supports Model 7 and Model 14 only; Johnson-Neyman and latent models are outside current scope. Model 14 still needs deterministic CI reproduction and separate coverage evidence. The slow Model 7 smoke is not cross-engine parity. | Add Model 14 deterministic CI reproduction, missing-row parity, controlled-index cross-engine moderated-mediation references, and Model 14 coverage smoke. |
...
| `compare_groups_t` | Module tests cover Welch and Student t paths; NIST StRD `AtmWtAg` anchors the two-group Student path through certified `F = t^2`. | Welch-first routing, custom/classic Student routing, common-offset centered t-family computation, signed Cohen's d, case-count reporting, row-order stability, and NIST `AtmWtAg` certified F parity. | `anova_oneway` still intentionally rejects two-treatment ANOVA; `AtmWtAg` is imported only through the compare-groups t-test identity. | Add a Welch-specific large-offset fixture if a certified two-group unequal-variance source is selected. |
...
- Local calculation-reliability gate:
  ...
  `launch-smoke-ok`, full pytest `993 passed, 5 skipped`, `pip check`, and
  slow statistics pytest `2 passed, 996 deselected`.
- Host quality gate:
...
- Slow bootstrap adequacy smoke:
  `scripts\slow_stats_gate.py` reported `2 passed, 996 deselected` for
  simple mediation percentile CI and moderated-mediation Model 7 index
...
for simple mediation and Model 7 index CIs, but it is not exhaustive proof and
does not replace controlled-index cross-engine references.
