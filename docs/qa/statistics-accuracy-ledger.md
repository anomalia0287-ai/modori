...
| `anova_oneway` | SciPy `f_oneway` parity tests and posthoc coverage via statsmodels/Pingouin paths. | Group validation, Levene assumption summaries, omnibus ANOVA, posthoc result surfaces, Tukey/Games-Howell p-value source disclosure, studentized-range survival-function policy lock. | NIST StRD ANOVA fixtures and independent studentized-range edge fixtures are not ledger-complete. | Add StRD ANOVA fixtures and extreme-tail posthoc fixtures for Tukey/Games-Howell. |
