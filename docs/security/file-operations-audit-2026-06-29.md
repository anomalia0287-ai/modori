...
| `scripts/package_environment.py` | `lstat`, `mkdir`, `resolve` | Shared release-only environment boundary. It removes workspace R runtime directories from packaged build/run `PATH` values so R DLLs cannot contaminate the PySide6 package. The lifecycle creates the unique test-owned state root and sentinel, then supplies an absolute lexical path. Before any resolution, the helper validates every existing component component-by-component with `lstat` without following links and rejects symlink/junction/reparse points and non-directories. It routes cache, Matplotlib, and settings below the validated root but does not precreate installed runtime children, so later existence proves child-runtime exercise. Standalone smokes retain their isolated workspace `.tmp` setup. It does not select or clean product user data. |
...
| `scripts/package_engine_smoke.py` | `mkdir`, `resolve`, `unlink` | Local packaged-engine smoke setup under the workspace `.tmp/packaged-engine-smoke` directory. It creates a deterministic reference workbook, deletes only its own previous `result.json` before launch to prevent stale success, constructs the child environment once, and requires the fresh packaged-app reported cache path to equal resolved `MODORI_CACHE_DIR`. It then requires the lexical expected cache to be a real directory and not a link/junction before accepting the smoke. It does not delete user data. |
