...
| TM-004 | Oversized or adversarial file | Profiler or model adapter lacks independent budgets | Exhaust CPU, memory, context, or UI responsiveness | Denial of service or lost work | Availability, project state | Preview and untrusted-project limits exist (`PreviewReadLimits`, `DEFAULT_UNTRUSTED_DATASET_LIMITS`) | Question/fingerprint budgets are designed but not implemented; model-runtime budgets remain manifest-gated | Enforce the designed count/time/cell budgets, cap model tokens and output size, run cancellably, and use deterministic abstention when a budget is exceeded | Record budget-exceeded codes and profile duration/peak sizes | High | Medium | High |
