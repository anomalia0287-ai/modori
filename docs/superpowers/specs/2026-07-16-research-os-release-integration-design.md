...
**Status:** P0 direction and written specification approved
...
### Integration-discovered branch-only adjustments (2026-07-17)

The original verifier treated every release-only and Research-OS-only blob as
immutable in the merge tree. Runtime integration exposed a legitimate counterexample:
preserving explicit candidate selection changes the setup required by a release-only
guidance test, and packaging the explanation library requires a stronger release-only
launch smoke. Hiding those adaptations in a later commit would make the merge verifier
describe a tree other than the accepted product tree. The final index audit exposed a
third case: the Research-OS-only string-catalog test must assert the frozen release
lane's `CASUAL MODE` labels after the shared string catalog is semantically merged.
That change preserves an exact assertion; it does not weaken or delete one.

The exact-blob rule remains the default. A branch-only path may differ in the merge
commit only when conflict-ledger schema v2 contains one closed
`branch_only_adjustments` record with all of the following: exact path and source side,
the pinned source blob SHA-1, the integrated blob SHA-1, a non-empty rationale,
required presences and absences, focused tests, and `status: verified`. The verifier
rejects unrecorded mismatches, duplicate or unknown paths, wrong source identity,
forged integrated hashes, incomplete records, and adjustments declared for an
unchanged blob. This mechanism is path-specific; it is not permission to prefer a
branch wholesale or to change a test identifier.

### Facade-budget preservation

Semantic union may exceed a release architecture budget even when both parents pass
independently. The integration must treat that as a product-structure failure, not as
permission to increase the threshold. For this merge, the 557-line release
`UiController` plus Research OS selection-origin contracts produced a 576-line facade.
The accepted repair extracts only the stateless QML report-export adapters to an
integration-authored `ReportExportControllerMixin`; it leaves the stateful export,
confirmation, provenance, and service-result boundary in `UiController`. The original
560-line, 60-method, and 45-line-method limits remain unchanged, and both API ownership
and end-to-end QML report export require regression evidence.
...
- every branch-only path must exist with its pinned content unless conflict-ledger v2
  records and hash-binds a verified integration adjustment under the closed protocol
  above;
