...
- Run whitespace checks over integration-authored, shared-resolution, and explicitly adjusted paths. Exact unchanged parent-only blobs are hash-verified and are not reformatted merely because the first-parent diff re-exposes pre-existing Markdown whitespace.
...
### Branch-only adjustment checkpoint (2026-07-17)

The first packaged human-operated recommendation flow exposed a missing knowledge
library in the PyInstaller output. The same audit showed that the original
"all branch-only blobs remain byte-identical" verifier rule was too coarse: explicit
candidate selection legitimately changes setup in a release-only guidance test, and a
release-only launch smoke must learn to reject the newly discovered package omission.
The final index audit also proved that a Research-OS-only string-catalog test must bind
the frozen release lane's `CASUAL MODE` copy instead of reasserting the superseded
Research OS labels.

Do not evade this finding with a post-merge fix commit. Conflict-ledger schema v2 adds
a closed `branch_only_adjustments` list. Each record binds the exact path, source side,
source blob, integrated blob, rationale, required presences and absences, focused tests,
and verified status. The verifier must reject unrecorded, duplicate, unknown, forged,
incomplete, or unnecessary adjustments. The expected adjustment set is exactly:

```text
docs/superpowers/plans/2026-07-16-research-os-release-integration.md
docs/superpowers/specs/2026-07-16-research-os-release-integration-design.md
scripts/package_launch_smoke.py
scripts/verify_release_integration.py
tests/test_knowledge_library.py
tests/test_package_launch_smoke_script.py
tests/test_verify_release_integration_script.py
tests/ui/test_experimental_guidance_boundary.py
tests/ui/test_experimental_recommendation_flow.py
tests/ui/test_qml_string_catalog.py
```

Any eleventh path is a hard stop until this checkpoint and its evidence are amended.
Parent test identifiers remain mandatory, and adjustments may not weaken assertions.
The Task 1 ledger-v1 scaffold below is historical; the operational contract for Tasks
9 and 10 is ledger v2 plus these fail-closed adjustment tests.

### Facade-budget checkpoint (2026-07-17)

The first complete integration quality gate stopped after `2472 passed, 5 skipped`
because `test_ui_controller_stays_within_facade_size_budget` measured a 576-line
`UiController` against the frozen 560-line ceiling. Both parents passed independently:
the release facade was already 557 lines, and the semantic union added the Research OS
selection-origin API. Raising the ceiling or compressing formatting would hide the
integration defect rather than resolve it.

The bounded correction creates integration-authored
`src/modori/ui/report_export_controller.py` and moves only the three stateless QML
report-export adapters into `ReportExportControllerMixin`. The stateful `exportReport`
boundary, local selection-origin injection, confirmation rejection, and service result
handling remain in `UiController`. A failing-first ownership test in the integration
boundary suite must precede the move. Acceptance is the unchanged 560/60/45 guard,
the complete report-export and QML regression cohort, and measured facade values no
higher than 531 lines, 53 methods, and a 44-line longest method. This is a new
integration file, not an eleventh parent-only adjustment; the closed adjustment set
remains exactly ten paths.
...
- `src/modori/ui/report_export_controller.py`: integration-authored QML report-export adapter mixin required to preserve the frozen facade budget without changing behaviour.
...
- `recommendation_controller.py`: retain the release candidate-kind and prepared-variable surface, but not the removed confidence-shaped `recommendationLevel`, `recommendationAlternativesText`, or `recommendationCandidateLevelAt` APIs; combine it with the Research OS preparation/confirmation/review-requirement state machine while keeping it a legacy experimental recommendation controller rather than a passport source;
...
- Stage: all automatically merged paths, every resolved conflict from Tasks 5 through 8, and exactly the ten hash-bound branch-only adjustments listed above.
...
$LedgerPayload = Get-Content -Raw `
    'docs\qa\research-os-release-integration-conflict-ledger.json' |
    ConvertFrom-Json
$AdjustedPaths = @($LedgerPayload.branch_only_adjustments.path)
$ExactBranchOnly = @(
    $ManifestPayload.release_only.path
    $ManifestPayload.research_only.path
) | Where-Object { $_ -notin $AdjustedPaths } | Sort-Object -Unique
$IntegrationReviewedPaths = @(
    git diff --cached --name-only |
        Where-Object { $_ -notin $ExactBranchOnly }
)
git diff --check
if ($LASTEXITCODE -ne 0) { throw 'Unstaged whitespace check failed' }
git diff --cached --check -- $IntegrationReviewedPaths
if ($LASTEXITCODE -ne 0) { throw 'Reviewed-path whitespace check failed' }
...
$Adjustments = @($LedgerPayload.branch_only_adjustments)
if ($LedgerPayload.schema_version -ne 2 -or $Adjustments.Count -ne 10) {
    throw 'Branch-only adjustment ledger is incomplete'
}
```

Expected: no unmerged entries, no diff-check output, exactly 38 verified shared-path entries, exactly 10 verified hash-bound branch-only adjustments, and no blank required evidence fields.
...
& $Python -m pytest `
    ...
    tests/ui/test_release_research_os_integration_boundary.py `
    tests/ui/test_architecture_guards.py `
    tests/ui/test_controller.py `
    tests/ui/test_human_operated_qml_flow.py `
    tests/ui/test_recommendations.py `
    tests/ui/test_qml_runtime_load.py `
...
Expected: `ok: true`, exact parent SHAs, 10 accepted hash-bound branch-only adjustments, zero unledgered branch-only blob mismatch, zero missing parent test ID, zero merge residue, and `shared_path_count: 38`.
